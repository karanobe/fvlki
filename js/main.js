jQuery(document).ready(function($){
	function ProductBuilder( element ) {
		...
		this.styles = this.element.find('[data-selection="styles"]'); 
		this.summary;
	...
	ProductBuilder.prototype.bindEvents = function() {
		...
		this.stepsWrapper.on('click', '.cd-product-vest-customizer a', function(event){
			event.preventDefault();
			self.customizeModelAlt($(this));
		});		
	};
	...
	ProductBuilder.prototype.newContentSelected = function(nextStep) {
		...
		if( this.fixedSummary.hasClass('disabled') ) {
			...
			if( this.steps.filter('.active').is('[data-selection="vests"]') ) {
				//in this case, color has been changed - update the preview image
				var imageSelected = this.steps.filter('.active').find('.cd-product-previews').children('.selected').children('img').attr('src');
				this.modelPreview.attr('src', imageSelected);
			}			
	...
	ProductBuilder.prototype.createSummary = function() {
		...
		this.steps.each(function(){
			...
			} else if( $(this).data('selection') == 'vests' ) {
				console.log('test');
				//create the Color summary
				var colorSelected = $(this).find('.cd-product-vest-customizer').find('.selected'),
					color = colorSelected.children('a').data('vest-color'),
					colorName = colorSelected.data('content'),
					imageSelected = $(this).find('.cd-product-previews').find('.selected img').attr('src');
	...
	ProductBuilder.prototype.updateListOptions = function(listItem) {
		...
		if( listItem.hasClass('js-radio') ) {
			...
			(listItem.parents('[data-selection="styles"]').length == 0) && self.updatePrice(price);
		} else {
		...
		if( listItem.parents('[data-selection="styles"]').length > 0 ) {
	...
	ProductBuilder.prototype.updateModelContent = function(model) {
		...
		if( model.hasClass('selected') ) {
			...
			this.styles.siblings('li').remove();
			...
			this.styles.find('.loaded').removeClass('loaded');
		}
	};
	...
	ProductBuilder.prototype.customizeModelAlt = function(target) {
		var parent = target.parent('li')
			index = parent.index();
		...
		var price = ( parent.hasClass('selected') )
			? 0
			: Number(parent.data('vest-price')) - parent.siblings('.selected').data('vest-price');

		this.updatePrice(price);
		target.parent('li').addClass('selected').siblings().removeClass('selected').parents('.cd-product-vest-customizer').siblings('.cd-product-previews').children('.selected').removeClass('selected').end().children('li').eq(index).addClass('selected');
	};	
